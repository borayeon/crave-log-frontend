  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-500 pb-24 md:pb-0 bg-[#F8FAFC] relative w-full">
      ...
      <div className="flex-1 px-6 md:px-10 py-6 overflow-y-auto scrollbar-hide">
        ...
        {/* ⭐️ 컴팩트하고 일관된 사이즈의 그리드 레이아웃 설정! */}
        <div className={isListMode 
            ? "flex flex-col gap-3 pb-10 max-w-4xl mx-auto w-full"
            : "grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-[repeat(auto-fill,minmax(160px,1fr))] gap-4 sm:gap-6 pb-10 justify-items-center w-full items-start"
        }>
            {customOrderedRecords.map((item, index) => {
                
                // 1. 음악 레코드판 UI (모든 탭에서 작고 예쁜 사이즈 유지)
                if (item.isMusic) {
                  ...
                  return (
                    // ⭐️ 강제 크기 제한 (max-w-[160px] sm:max-w-[180px]) 으로 '음악' 탭처럼 아담하게 고정
                    <div key={item.id} onClick={() => !isEditing && setSelectedRecord(item)} className={`group relative w-full max-w-[160px] sm:max-w-[180px] mx-auto flex flex-col items-center justify-start cursor-pointer animate-in fade-in transition-all duration-500 ease-out ${!isEditing ? 'hover:-translate-y-1' : ''}`}>
                      <div className={`relative w-full aspect-square rounded-full overflow-hidden shadow-lg border-[3px] border-zinc-900 transition-transform duration-500 ease-out ${isEditing ? 'opacity-80 scale-100' : 'group-hover:scale-105 group-hover:shadow-xl group-hover:border-zinc-800'}`}>
                        ...
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 md:w-12 md:h-12 bg-zinc-900 rounded-full border-[2px] border-zinc-700 flex items-center justify-center shadow-inner">
                            <PlayCircle size={16} className="text-white/80 translate-x-[1px]" />
                        </div>
                        {isEditing && (
                            <button onClick={(e) => { e.stopPropagation(); setConfirmDeleteId(item.id); }} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-30 p-2.5 bg-rose-500 text-white rounded-full shadow-lg hover:bg-rose-600 hover:scale-110 transition-all animate-in zoom-in-50">
                                <Trash2 size={16} />
                            </button>
                        )}
                      </div>
                      <div className="mt-3 text-center px-1 w-full">
                        <h3 className="text-xs sm:text-sm font-black text-zinc-800 line-clamp-1 group-hover:text-indigo-600 transition-colors">{item.title}</h3>
                        <p className="text-[9px] font-bold text-red-500 mt-1 flex items-center justify-center gap-1 uppercase tracking-widest"><Disc size={10} /> Record</p>
                      </div>
                      {!item.isPublic && !isEditing && <div className="absolute top-0 right-0 p-1 bg-zinc-900/80 backdrop-blur-md text-rose-400 rounded-full shadow-sm z-20"><Lock size={10} /></div>}
                    </div>
                  );
                }
                ...
                // 2. 일반 카드 (URL 및 이미지 뷰)
                return (
                  // ⭐️ 강제 크기 제한 (max-w-[160px] sm:max-w-[180px]) 으로 일정하게 고정
                  <div key={item.id} onClick={() => { 
                  ...
                  }} className={`group relative aspect-[3/4] w-full max-w-[160px] sm:max-w-[180px] mx-auto rounded-xl sm:rounded-2xl overflow-hidden shadow-sm cursor-pointer border border-zinc-200/80 bg-white transition-all duration-500 ease-out transform flex flex-col ${!isEditing ? 'hover:scale-[1.03] hover:-translate-y-1 hover:shadow-md hover:z-10 hover:border-indigo-300' : ''}`}>
                      {item.isUrlItem ? (
                          <div className="w-full h-full bg-gradient-to-br from-blue-50/80 via-white to-zinc-50/80 flex flex-col items-center justify-center p-4 text-center relative overflow-hidden group/card">
                              <div className="absolute top-2 left-2 px-1.5 py-0.5 bg-blue-100/60 text-blue-700 text-[8px] font-black rounded flex items-center gap-1 shadow-sm z-10"><LinkIcon size={10} /> {item.category}</div>
                              <div className="flex flex-col items-center w-full h-full justify-center mt-3">
                                  {item.youtubeUrl && item.domain ? (
                                      <div className="w-10 h-10 sm:w-12 sm:h-12 bg-white rounded-xl shadow-sm flex items-center justify-center p-1.5 mb-2 border border-zinc-100 transition-transform group-hover/card:scale-110">
                                          <img 
                                  ...
                                  ) : <div className="w-10 h-10 sm:w-12 sm:h-12 bg-blue-50 text-blue-300 rounded-xl flex items-center justify-center mb-2 transition-transform group-hover/card:scale-110"><LinkIcon size={20} className="sm:w-6 sm:h-6" /></div>}
                                  <h3 className="text-xs sm:text-sm font-black text-zinc-800 mb-1 leading-snug line-clamp-2 px-1 w-full">{item.title}</h3>
                                  <p className="text-[8px] sm:text-[9px] font-bold text-zinc-400 mt-1 bg-white/80 px-1.5 py-0.5 rounded border border-zinc-100 truncate max-w-[90%]" >{item.domain || '클릭하여 열기'}</p>
                              </div>
                              <div className="absolute inset-0 flex flex-col items-center justify-center opacity-0 transition-opacity duration-300 group-hover/link:opacity-100 bg-blue-50/95 backdrop-blur-sm">
                                  <ExternalLink size={20} className="text-blue-500 mb-1" />
                                  <span className="text-[9px] font-black text-blue-600 bg-white px-2 py-1 rounded shadow-sm hidden sm:block">새 창 열기</span>
                              </div>
                          </div>
                      ) : item.isTextOnly ? (
                          <div className="w-full h-full bg-gradient-to-br from-indigo-50/60 via-white to-zinc-50/60 flex flex-col items-center justify-center p-4 text-center relative overflow-hidden">
                              <div className="absolute top-2 left-2 px-1.5 py-0.5 bg-indigo-100/60 text-indigo-600 text-[8px] font-black rounded flex items-center gap-1 shadow-sm"><Quote size={8} /> {item.category}</div>
                              <h3 className="text-sm sm:text-base font-black text-zinc-800 mb-1 mt-3 leading-snug group-hover:text-indigo-600 transition-colors line-clamp-2 px-1">{item.title}</h3>
                              {item.content && <p className="text-[9px] sm:text-[10px] font-medium text-zinc-500 line-clamp-4 leading-relaxed px-2">"{item.content}"</p>}
                          </div>
                      ) : (
                          <>
                            <div className="relative w-full flex-1 overflow-hidden bg-zinc-100">
                                ...
                                <div className="absolute top-2 left-2 px-1.5 py-0.5 bg-white/90 backdrop-blur-md text-indigo-600 text-[8px] font-black rounded shadow-sm uppercase tracking-wider">{item.category}</div>
                            </div>
                            <div className="p-2 sm:p-3 flex flex-col justify-center bg-white z-10 border-t border-zinc-100 shrink-0 h-[50px] sm:h-[60px]">
                                <h4 className="text-[11px] sm:text-xs font-black text-zinc-900 truncate group-hover:text-indigo-600 transition-colors">{item.title}</h4>
                                {(item.tags || []).length > 0 && (
                                  <div className="mt-1 flex flex-wrap gap-1 overflow-hidden h-3 sm:h-4">
                                    {item.tags.slice(0, 2).map(tag => <span key={tag} className="text-[8px] font-bold text-zinc-400 truncate max-w-[50px] sm:max-w-[60px]">#{tag}</span>)}
                                  </div>
                                )}
                            </div>
                          </>
                      ...
                      {!item.isPublic && <div className="absolute top-2 right-2 p-1.5 bg-zinc-900/80 backdrop-blur-md text-rose-400 rounded-full shadow-sm z-20"><Lock size={10} /></div>}
                      
                      {isEditing && (
                          <button onClick={(e) => { e.stopPropagation(); setConfirmDeleteId(item.id); }} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-30 p-2.5 bg-rose-50 text-white rounded-full shadow-lg hover:bg-rose-600 hover:scale-110 transition-all animate-in zoom-in-50">
                              <Trash2 size={16} />
                          </button>
                      )}
                  </div>
